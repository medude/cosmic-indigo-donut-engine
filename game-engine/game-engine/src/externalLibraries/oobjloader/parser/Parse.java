package externalLibraries.oobjloader.parser;
    ...
    private void parseObjFile(String objFilename) throws FileNotFoundException, IOException {
        ...
        while (true) {
            ...
            if (line.startsWith("#")) // comment
            {
                ...
                console.log("Warning: line " + lineCount + " unknown line |" + line + "|");
            }
            lineCount++;
        }
...
//        System.err.println("Parsed as "+parsedList);
        ...
        if (null != matlibnames) {
            for (int loopi = 0; loopi < matlibnames.length; loopi++) {
                try {
                    ...
                    console.error("Can't find material file name='" + matlibnames[loopi] + "', e=" + e);
                }
            }
        }
    }
    ...
    private void parseMtlFile(String mtlFilename) throws FileNotFoundException, IOException {
        ...
        while (true) {
            ...
            if (line.startsWith("#")) // comment
            {
                ...
                console.warn("line " + lineCount + " unknown line |" + line + "|");
        ...
        console.log("Parse.parseMtlFile: Loaded " + lineCount + " lines");
    }
    ...
    private void processReflectivityTransmissivity(String fieldName, String line) {
        ...
        if (null == tokens) {
            console.error("Got Ka line with no tokens, line = |" + line + "|");
            return;
        }
        if (tokens.length <= 0) {
            console.error("Got Ka line with no tokens, line = |" + line + "|");
        ...
        if (tokens[0].equals("spectral")) {
            ...
            console.warn("Sorry Charlie, this parse doesn't handle \'spectral\' parsing.  (Mostly because I can't find any info on the spectra.rfl file.)");
            return;
// 	    if(tokens.length < 2) {
...
// 	    }
            ...
            if (tokens.length < 2) {
                console.error("Got xyz line with not enough x/y/z tokens, need at least one value for x, found " + (tokens.length - 1) + " line = |" + line + "|");
                return;
            }
    ...
    private void processIllum(String line) {
        ...
        if ((illumModel < 0) || (illumModel > 10)) {
            console.error("Got illum model value out of range (0 to 10 inclusive is allowed), value=" + illumModel + ", line=" + line);
            return;
        }
    ...
    private void processRefl(String line) {
        ...
        if (line.startsWith("-type")) {
            ...
            if (line.startsWith(MTL_REFL_TYPE_SPHERE)) {
                ...
                console.error("unknown material refl -type, line = |" + line + "|");
                return;
            }
        } else {
            filename = line;
