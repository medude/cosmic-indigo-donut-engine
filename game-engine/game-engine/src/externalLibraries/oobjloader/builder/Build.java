...
import externalLibraries.oobjloader.parser.BuilderInterface;
import services.Services;
import services.console.CoreConsole;

public class Build implements BuilderInterface {

    private CoreConsole console=Services.getConsole();
...
//        log.log(INFO,"Added texture  vertex " + verticesT.size() + " = " + verticesT.get(verticesT.size() - 1));
    ...
    public void addPoints(int[] values) {
        console.log("@TODO: Got " + values.length + " points in builder, ignoring");
    }

    public void addLine(int[] values) {
        console.log("@TODO: Got a line of " + values.length + " segments in builder, ignoring");
    }
...
//            log.log(INFO,"Adding vertex g=" + vertexIndices[loopi] + " t=" + vertexIndices[loopi + 1] + " n=" + vertexIndices[loopi + 2]);
            ...
            if (((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesG.size())) {
            	...
            	console.error("Index for geometric vertex=" + vertexIndex + " is out of the current range of geometric vertex values 1 to " + verticesG.size() + ", ignoring");
            }
            ...
            if (vertexIndex != EMPTY_VERTEX_VALUE) {
                ...
                if (((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesT.size())) {
                    ...
                    console.error("Index for texture vertex=" + vertexIndex + " is out of the current range of texture vertex values 1 to " + verticesT.size() + ", ignoring");
                }
            }
            ...
            if (vertexIndex != EMPTY_VERTEX_VALUE) {
                ...
                if (((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesN.size())) {
                	...
                	console.error("Index for vertex normal=" + vertexIndex + " is out of the current range of vertex normal values 1 to " + verticesN.size() + ", ignoring");
                }
            }

            if (fv.v == null) {
            	console.error("Can't add vertex to face with missing vertex!  Throwing away face.");
                faceErrorCount++;
                return;
            }
...
//        log.log(INFO,"Parsed face=" + face);
    ...
    public void addMapLib(String[] names) {
        if (null == names) {
        	console.log("@TODO: ERROR! Got a maplib line with null names array - blank group line? (i.e. \"g\\n\" ?)");
            return;
        }
        if (names.length == 1) {
        	console.log("@TODO: Got a maplib line with one name=|" + names[0] + "|");
            return;
        }
        console.log("@TODO: Got a maplib line;");
        for (int loopi = 0; loopi < names.length; loopi++) {
        	console.log("        names[" + loopi + "] = |" + names[loopi] + "|");
        }
    }
...
//    }
    ...
    public void doneParsingObj(String filename) {
    	console.log("Loaded filename '" + filename + "' with " + verticesG.size() + " verticesG, " + verticesT.size() + " verticesT, " + verticesN.size() + " verticesN and " + faces.size() + " faces, of which " + faceTriCount + " triangles, " + faceQuadCount + " quads, and " + facePolyCount + " with more than 4 points, and faces with errors " + faceErrorCount);
    }
}
