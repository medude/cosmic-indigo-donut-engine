	...
	public void run() {
		try {
			...
			Group group2 = new Group( // Create group2
					new Thing[] { thing2, thing3 } // Assign thing2 and thing3
													// as children
			);
			...
			// Move area2
			area2.updateComponent("LocalTransformComponent", new TransformComponent(
					TransformationMatrix.create(new Vector3(0, -1, -5), new Vector3(90, 0, 0), 1)));
			...
			while (WindowManager.testForClose(window)) {
				...
				// Move group
				group.updateComponent("LocalTransformComponent", new TransformComponent(
						TransformationMatrix.create(new Vector3(0, 0, -rot), new Vector3(90, rot, 0), 1)));
